...
def cross_sectional_rank_normalize(
    ...
    *,
    ...
    """Return a copy of ``frame`` with each named feature replaced by its
    per-date cross-sectional percentile rank in ``[0, 1]``.

    **Why this exists.** A linear ranker scores ``Σ feature·weight``. Raw
    features span wildly different magnitudes — e.g. ``dollar_volume_20d`` is
    raw dollars (~1e8) while returns are ~[-1, 1] — so a *raw* weighted sum is
    dominated by the largest-scale feature regardless of its IC weight,
    collapsing a multi-factor model into a single-feature sort. Mapping every
    feature to a common per-date rank scale makes each feature contribute in
    proportion to its weight. It is also consistent with IC weights fit by
    rank-based Spearman correlation: that fit is invariant to this monotonic
    transform, so the *fitted weights are unchanged* — only the scoring is
    corrected.

    Each feature is ranked within its ``date_column`` group via
    ``rank(method="average", pct=True)`` (ties share the mean rank). The
    ``[0, 1]`` (rather than centered) range is deliberate: for non-negative
    weights summing to 1 the resulting score lands in ``[0, 1]``, so a
    downstream clamp to ``[-1, 1]`` is a no-op and long-only weighting
    (``score / Σ|score|``) stays all-positive. NaN ranks (missing/non-finite
    inputs, or a single-name date) are filled with ``fill_value`` (default 0.5
    = the neutral median rank) so the weighted sum is always well-defined.
    The input frame is not mutated; non-feature columns pass through unchanged.
    """
    out = frame.copy()
    grouped = out.groupby(date_column, sort=False)
    for name in feature_names:
        out[name] = grouped[name].rank(method="average", pct=True).fillna(fill_value)
    return out
