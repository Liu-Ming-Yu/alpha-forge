"""Promotion guard for event candidate screens."""
...
def promote_event_candidate_screens(
    *,
    ...
    """Write tracked metadata only for candidates shared by all event screens."""
    screens = {
        "main": main_screen,
    ...
    if feature_set is None:
        blockers.append("screen event_feature_set_version values do not match")
        feature_set = str(full_screen.get("event_feature_set_version", ""))
    if len(shared_known) < min_passing_candidates:
        blockers.append(
            "event promotion shared "
    ...
    base = {
        ...
        "event_feature_set_version": feature_set,
    }
    if blockers:
        return {
            **base,
            "passed": False,
            "reason": "event promotion blocked tracked feature metadata",
            "blockers": blockers,
    ...
    artifacts = write_event_candidate_family_artifacts(
    ...
    return {
        **base,
        "passed": True,
        "reason": "event promotion passed shared-candidate rule",
        "blockers": [],
...
def _common_feature_set(screens: Mapping[str, Mapping[str, object]]) -> str | None:
    values = {
        str(screen.get("event_feature_set_version", "")).strip() for screen in screens.values()
    }
    values.discard("")
    if len(values) != 1:
        return None
...
__all__ = ["promote_event_candidate_screens"]
